...
- **Typage des ids : `Snowflake` partout** (harmonisé en 2026-07-21). Tous les modèles (`Guild`/`Role`/`User`/`Member`/`Message`/`Emoji`…) et tous les paramètres d'id des fonctions REST utilisent `Snowflake`. Restent en `String`/`Int` les identifiants non-snowflake : tokens, `custom_id`, bitfields de permissions, `session_id`, `ActivityParty.id`. `Snowflake` est une inline class ⇒ pas de `lateinit` possible (cf. `DiscordClient.applicationId`, initialisé par une sentinelle dans `create()`).
